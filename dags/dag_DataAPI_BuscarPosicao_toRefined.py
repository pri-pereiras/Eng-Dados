...
def save_BuscarPosicao_toRefined(**kwargs):
    ...
    for obj in arquivos.get('Contents', []):
        ...
        path_refined = f"veiculos-posicao/ano={ano}/mes={mes}/dia={dia}/{nome_arquivo}"
...
with DAG('DataAPI_BuscarPosicao_toRefined', default_args=default_args, 
    ...
    move_data_task = PythonOperator(
        task_id='save_BuscarPosicao_toRefined',
        python_callable=save_BuscarPosicao_toRefined
    )

    # Trigger para a próxima DAG
    trigger_postgres = TriggerDagRunOperator(
        task_id='trigger_postgres',
        trigger_dag_id='DataAPI_BuscarPosicao_toPostgres',
